...
### `k8s/`
Contains the Kubernetes runtime scope. The current base includes namespace, shared configuration, API and frontend service manifests, plus a dev overlay for local PostgreSQL, Redpanda, migrations and seed data; ingress and Helm charts are not implemented yet.
...
* AWS Terraform foundation and CI/CD automation are implemented; Kubernetes workload deployment remains future scope beyond the initial API/frontend, dev database/broker and one-shot job manifests.
...
Kubernetes is a target runtime design. The repository now includes base manifests for namespace, shared runtime configuration, API service, frontend service, local dev PostgreSQL, Redpanda, migrations and seed data, not a complete deployment path.
