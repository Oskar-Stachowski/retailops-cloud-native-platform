...
def _rng(seed: int, profile: str) -> random.Random:
    return random.Random(f"retailops-{profile}-{seed}")


def _demand_class(index: int, product_count: int) -> str:
    percentile = (index + 1) / max(product_count, 1)
    if percentile <= 0.08:
        return "hero_product"
    if percentile <= 0.25:
        return "core_product"
...
def _seasonal_multiplier(pattern: str, day_index: int) -> Decimal:
    ...
    if pattern == "holiday_peak" and day_of_year >= 315:
        return Decimal("1.65")
    if pattern == "spring_summer_peak" and 90 <= day_of_year <= 220:
        return Decimal("1.40")
    if pattern == "seasonal_peak" and day_of_year in range(250, 360):
        return Decimal("1.55")
    if pattern == "stable":
        return Decimal("1.00")
    return Decimal("1.08") if day_of_year % 30 in range(0, 5) else Decimal("1.00")
...
def _stock_limit(
    ...
    if product["demand_class"] == "hero_product" and day_index % 31 in range(0, 4):
...
def _basket_size(channel: str, rng: random.Random) -> int:
    ...
    if channel == "online":
        return 1 if roll < 0.35 else 2 if roll < 0.82 else 3
                ...
                sales.append(
                    {
                        ...
                        "ingested_at": _date_at_offset(
                            day_index,
                            (product_index + item_index) % 12 + (2 if data_quality_status == "late_event" else 0),
                        ),
                    }
                )
...
def generate_profile_stock_movements(
    ...
    for snapshot in inventory_snapshots:
        ...
        if day_value.endswith("-30") or day_value.endswith("-01"):
        ...
        forecasts.append(
            {
                ...
                "status": "generated",
                "confidence_level": str(max(Decimal("0.55"), confidence).quantize(Decimal("0.0001"))),
            }
        )
